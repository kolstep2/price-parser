...
STANDARD_FIELDS = [
    "supplier","sheet","header_row",
    "name_col",
    "size_col",
    "length_col","width_col","height_col",
    "price_col","price_cols","price_labels",
    "currency_col","sku_col","qty_col","uom_col",
    "regex_extract_size_from","size_regex","price_regex","price_multiplier",
    "drop_na_price","skip_top_rows","skip_bottom_rows","filters","output_renames",
]
...
class PriceTemplate:
    ...
    # dimensions (optional explicit mapping)
...
def apply_template(df: pd.DataFrame, t: PriceTemplate) -> pd.DataFrame:
    ...
    # labels for output price columns
    ...
    if t.filters:
        ...
        if isinstance(f, dict):
            # aggregate helper over all price columns
...
with cb:
    t.size_col = st.selectbox("Размер (строка, если есть)", options=["— нет —"] + cols, index=(cols.index(t.size_col) + 1) if t.size_col in cols else 0)
    t.length_col = st.selectbox("Длина (если есть)", options=["— нет —"] + cols, index=(cols.index(t.length_col) + 1) if t.length_col in cols else 0)
    t.width_col  = st.selectbox("Ширина (если есть)", options=["— нет —"] + cols, index=(cols.index(t.width_col) + 1) if t.width_col in cols else 0)
    t.height_col = st.selectbox("Высота (если есть)", options=["— нет —"] + cols, index=(cols.index(t.height_col) + 1) if t.height_col in cols else 0)
with cc:
    preselected_prices = t.price_cols or ([t.price_col] if (t.price_col and t.price_col in cols) else [])
    t.price_cols = st.multiselect("Цены (можно несколько)", options=cols, default=[c for c in preselected_prices if c in cols])
    labels: List[str] = []
    if t.price_cols:
        st.caption("Имена выходных столбцов цен (по умолчанию берём имена исходных столбцов):")
        for i, csrc in enumerate(t.price_cols):
            default_label = (t.price_labels[i] if (t.price_labels and i < len(t.price_labels) and t.price_labels[i]) else csrc)
            labels.append(st.text_input(f"Название для цены {i+1} ({csrc})", value=default_label, key=f"lbl_{i}_{csrc}"))
...
st.markdown("**Извлечение размера по шаблону (опционально):**")
    ...
    # Переименование выходных колонок (опционально)
    ...
    for fld in ["name_col","size_col","length_col","width_col","height_col","price_col","currency_col","sku_col","qty_col","uom_col"]:
        v = getattr(t, fld)
        if v == "— нет —":
            setattr(t, fld, None)
